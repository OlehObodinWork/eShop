...
public sealed class PrimaryCatalogAI : IPrimaryCatalogAI
{
    ...
    public PrimaryCatalogAI(IWebHostEnvironment environment, ILogger<PrimaryCatalogAI> logger, IEmbeddingGenerator<string, Embedding<float>> embeddingGenerator = null)
    {
    ...
    /// <inheritdoc/>
    public ValueTask<Vector> GetEmbeddingAsync(PrimaryCatalogItem item) =>
        IsEnabled ?
            GetEmbeddingAsync(CatalogItemToString(item)) :
            ValueTask.FromResult<Vector>(null);

    /// <inheritdoc/>
    public async ValueTask<IReadOnlyList<Vector>> GetEmbeddingsAsync(IEnumerable<PrimaryCatalogItem> items)
    {
        if (IsEnabled)
        {
    ...
    private static string CatalogItemToString(PrimaryCatalogItem item) => $"{item.Name} {item.Description}";
}
